		...
		public function incubiDraft(tainted:Boolean,player:Player):void
		{
			...
			if (rando < 50) {
				...
				if (player.cocks.length == 0) {
					...
					outputText("The skin seems to fold back as a fully formed demon-cock bursts forth from your loins, drizzling hot cum everywhere as it orgasms.  Eventually the orgasm ends as your " + player.cockDescript(0) + " fades to a more normal " + player.skin.tone + " tone.");
...
//5-Bulbous Pepper (+ball size or fresh balls)
		public function caninePepper(type:Number,player:Player):void
		{
			...
			//OVERDOSE Bad End!
			if (type <= 0 && crit > 1 && player.hasFur() && player.face.type == Face.DOG && player.ears.type == Ears.DOG && player.lowerBody.type == LowerBody.DOG && player.tailType == Tail.DOG && rand(2) == 0 && player.hasStatusEffect(StatusEffects.DogWarning) && player.findPerk(PerkLib.TransformationResistance) < 0) {
				temp = rand(2);
				if (temp == 0) {
			...
			if (type <= 0 && player.hasFur() && player.face.type == Face.DOG && player.tailType == Tail.DOG && player.ears.type == Ears.DOG && player.lowerBody.type == LowerBody.DOG && player.hasStatusEffect(StatusEffects.DogWarning) && rand(3) == 0) {
			...
			if (type <= 0 && player.hasFur() && player.face.type == Face.DOG && player.tailType == Tail.DOG && player.ears.type == Ears.DOG && player.lowerBody.type == LowerBody.DOG && !player.hasStatusEffect(StatusEffects.DogWarning)) {
        ...
        if (player.goIntoHeat(true)) {
          changes++;
			...
			if (rand(4) == 0 && changes < changeLimit && player.lowerBody.type == LowerBody.DOG && player.tailType == Tail.DOG && !player.hasFur()) {
			...
			if (rand(3) == 0 && player.lowerBody.type != LowerBody.DOG && player.tailType == Tail.DOG && player.ears.type == Ears.DOG && changes < changeLimit) {
				//Feet -> paws
...
//Nipplezzzzz
		...
		public function neonPinkEgg(pregnantChange:Boolean,player:Player):void
		{
			...
			//DAH BUNBUNTAILZ
			if (player.tailType != Tail.RABBIT && rand(2) == 0 && changes < changeLimit) {
		...
		public function goldenSeed(type:Number,player:Player):void
		{
			...
			//****************
			//-Harpy legs
			if (player.lowerBody.type != LowerBody.HARPY && changes < changeLimit && (type == 1 || player.tailType == Tail.HARPY) && rand(4) == 0) {
				//(biped/taur)
			...
			if (player.findPerk(PerkLib.HarpyWomb) < 0 && player.lowerBody.type == LowerBody.HARPY && player.tailType == Tail.HARPY && rand(4) == 0 && changes < changeLimit) {
		...
		public function kangaFruit(type:Number,player:Player):void
		{
			...
			if (player.lowerBody.type != LowerBody.KANGAROO && (type == 1 || player.tailType == Tail.KANGAROO) && changes < changeLimit && rand(4) == 0) {
			...
			if (player.tailType != Tail.KANGAROO && changes < changeLimit && rand(4) == 0 && (type == 1 || player.ears.type == Ears.KANGAROO)) {
				...
				else if (player.tailType == Tail.SPIDER_ABDOMEN || player.tailType == Tail.BEE_ABDOMEN) {
		...
		public function sweetGossamer(type:Number,player:Player):void
		{
			...
			if (player.tailType == Tail.SPIDER_ABDOMEN && player.tailRecharge < 25 && changes < changeLimit) {
				changes++;
			...
			if (rand(4) == 0 && changes < changeLimit && player.lowerBody.type != LowerBody.DRIDER_LOWER_BODY && player.lowerBody.type != LowerBody.CHITINOUS_SPIDER_LEGS) restoreLegs(tfSource);
			//Drider butt
			...
			if (((type == 1 && player.lowerBody.type != LowerBody.DRIDER_LOWER_BODY && player.lowerBody.type != LowerBody.CHITINOUS_SPIDER_LEGS) || (type != 1 && player.lowerBody.type != LowerBody.CHITINOUS_SPIDER_LEGS)) && (!player.isGoo() && !player.isNaga() && !player.isTaur()) && changes < changeLimit && rand(4) == 0) {
			...
			if (player.tailType != Tail.SPIDER_ABDOMEN && (player.lowerBody.type == LowerBody.CHITINOUS_SPIDER_LEGS || player.lowerBody.type == LowerBody.DRIDER_LOWER_BODY) && player.arms.type == Arms.SPIDER && rand(4) == 0) {
				outputText("\n\n");
				//(Pre-existing tails)
			...
			if (type == 1 && player.lowerBody.type == LowerBody.CHITINOUS_SPIDER_LEGS && rand(4) == 0 && player.tailType == Tail.SPIDER_ABDOMEN) {
				...
				player.lowerBody.type = LowerBody.DRIDER_LOWER_BODY;
...
//Mouseover script: \"The green-tinted, hardly corporeal substance flows like a liquid inside its container. It makes you feel...uncomfortable, as you observe it.\"
		...
		public function foxTF(enhanced:Boolean,player:Player):void
		{
			...
			if (player.face.type == Face.FOX && player.tailType == Tail.FOX && player.ears.type == Ears.FOX && player.lowerBody.type == LowerBody.FOX && player.hasFur() && rand(3) == 0 && player.findPerk(PerkLib.TransformationResistance) < 0) {
		...
		public function foxJewel(mystic:Boolean,player:Player):void
		{
			...
			if (rand(5) == 0) updateOvipositionPerk(tfSource);
			//[Grow Fox Tail]
			if (player.tailType != Tail.FOX && changes < changeLimit && ((mystic && rand(2) == 0) || (!mystic && rand(4) == 0))) {
				//if PC has no tail
			...
			if (!mystic && player.ears.type == Ears.FOX && player.tailType == Tail.FOX && player.tailVenom == 8 && rand(3) == 0) {
			...
			else if (player.tailType == Tail.FOX && player.tailVenom < 8 && player.tailVenom + 1 <= player.level && player.tailVenom + 1 <= player.inte / 10 && changes < changeLimit && ((mystic && rand(2) == 0) || (!mystic && rand(3) == 0))) {
			...
			else if (mystic && rand(4) == 0 && changes < changeLimit && player.tailType == Tail.FOX && player.tailVenom == 8 && player.level >= 9 && player.ears.type == Ears.FOX && player.inte >= 90 && player.findPerk(PerkLib.CorruptedNinetails) < 0 && (player.findPerk(PerkLib.EnlightenedNinetails) < 0 || player.perkv4(PerkLib.EnlightenedNinetails) > 0)) {
			...
			//[Grow Fox Ears]
			if (player.tailType == Tail.FOX && ((mystic && rand(2) == 0) || (!mystic && rand(4) == 0)) && player.ears.type != Ears.FOX && changes < changeLimit) {
		...
		private function demonChanges(player:Player):void
		{
			...
			if (player.tailType != Tail.DEMONIC && player.horns.value > 0) {
				if (player.tailType != Tail.NONE) {
					outputText("\n\n");
		...
		public function pigTruffle(boar:Boolean, player:Player):void
		{
			...
			if (rand(boar ? 2 : 3) == 0 && changes < changeLimit && player.ears.type == Ears.PIG && player.tailType != Tail.PIG && player.isTaur() && (player.lowerBody.type == LowerBody.HOOFED || player.lowerBody.type == LowerBody.PONY)) {
			...
			if (rand(boar ? 3 : 4) == 0 && changes < changeLimit && player.ears.type == Ears.PIG && player.tailType == Tail.PIG && player.lowerBody.type != LowerBody.CLOVEN_HOOFED) {
			...
			if (rand(boar ? 2 : 3) == 0 && changes < changeLimit && player.ears.type == Ears.PIG && player.tailType == Tail.PIG && player.lowerBody.type == LowerBody.CLOVEN_HOOFED && (player.face.type != Face.PIG && player.face.type != Face.BOAR)) {
			...
			if (rand(3) == 0 && changes < changeLimit && player.ears.type == Ears.PIG && player.tailType == Tail.PIG && player.lowerBody.type == LowerBody.CLOVEN_HOOFED && player.face.type == Face.PIG) {
