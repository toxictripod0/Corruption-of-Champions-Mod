		...
		public function lizardHairChange(tfSource:String):int
		{
			...
			switch (tfSource) {
				...
				case "reptilum-dracolisk": //never 4get the basilisk hair discourse of may 2017
					if (player.hairType == HAIR_BASILISK_PLUME && player.isPureEnough(65)) return 0;

					if (player.isFemaleOrHerm() && player.isPureEnough(15) && player.featheryHairPinEquipped() && player.isBasilisk()) {
					...
					if (player.isCorruptEnough(65) && player.hairType != HAIR_BASILISK_SPINES && player.hasLizardScales() && player.hasReptileFace()) {
						// Corrupted Basilisk
