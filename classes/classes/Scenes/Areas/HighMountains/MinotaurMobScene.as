...
public function meetMinotaurSons():void {
	...
	if (flags[kFLAGS.TIMES_ENCOUNTERED_MINOTAUR_MOB] == 1) {
		...
		if (!player.minotaurAddicted()) {
			...
			if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 3) outputText("some of ", false);
	...
	//Repeat meetings
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 20) {
			//[Non Addicted Repeat] 
			if (!player.minotaurAddicted()) {
				outputText("It doesn't take long for your exploration into the mountains to bring you face-to-face with your minotaur offspring once again.  There're " + num2Text(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS]) + " of them here, arranged around you in a loose semicircle.  You can smell their familiar and arousing odor in the air; while it brings a tingle to your nethers, you're determined to resist it.  ", false);
...
private function nonAddictMinotaurGangBang():void {
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 10) outputText("The assembled beasts snicker at the noise, stroking themselves slowly while awaiting a chance at your hungry holes.", false);
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] <= 3) outputText(" the minotaur that gave you your first dose.  Maybe he's ready to give you more? He smirks and slaps you across the face with his soft shaft.  You giggle drunkenly and start to lick his balls - they HAVE to have more for you!  ", false);
	else {
...
private function loseToMinoMobVeryPregnant():void {
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 8) {
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 3) {
			if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] == 4) outputText("Sorry bro, it looks like Mom's done.  You'll have to snag a shot at her later - she's pretty fucking pregnant, and I don't want her broken.", false);
...
private function analSpearSemiPregMinotaurGangbang():void {
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 20) outputText("to make the tribe even bigger", false);
	...
	if (player.hasFuckableNipples()) {
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 7) outputText("shakes his head.  \"<i>You're only saving up trouble for later, little cunt.  Your hole after Mom's, remember.</i>\"  The minitaur wistfully withdraws his cock and crawls a few paces off, where he sits in a sulk.", false);
...
private function victoryMinotaurGangTitFuck():void {
	...
	//(SMALL) 
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] == 3) outputText("one minotaur", false);
		else outputText(num2Text(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS]-2) + " minotaurs", false);
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] == 3) outputText("You grab the minitaur's cock and drag the shy beast-man over your cum-slicked tits, wasting no time in burying the shaft in mommy's mammaries, where it belongs.", false);
		else {
			outputText("You grab the minitaur and ", false);
			if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] == 4) outputText("the remaining brother", false);
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 3) outputText("His glaze spurts up along the opening created by his larger brother's shaft, and soon he too is cumming, obediently orgasming for you.  ", false);
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 5) outputText("The minotaurs let you get dressed while they sigh happily, clearly pleased with the outcome.", false);
		else {
			...
			if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 5) {
				...
				if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 6) outputText("s", false);
			}
...
private function victoryAllThePenetrationsMinotaurGangBang():void {
	...
	if (player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 4) outputText("  There's still more you could be doing.", false);
	outputText("\n\n", false);
	...
	if (player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) {
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] == 4) outputText("One stud assembles next to ", false);
		else if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 5 && player.totalBreasts() == 2) outputText("Two studs assemble around ", false);
		else {
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] == 4) outputText("He excitedly fingers", false);
	...
	if (player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) outputText("your nipples squirt, ", false);
	...
	if (player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) {
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) outputText("Meanwhile, your " + player.nippleDescript(0) + "s get a similar feeling from the studs arranged around them", false);
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 20) outputText("gang", false);
...
private function forceMinitaurToGiveOral(choice:Number = 0):void {
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 20) outputText("mob", false);
	else outputText("tribe", false);
...
private function victoryBJNippleFuckMinotaurGang():void {
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] == 3) outputText("the minitaur, much to the smaller monster's chagrin.", false);
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 7) outputText(", chased by jeers from the remainder of your brood", false);
	...
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 5) outputText("relax and fan themselves while giving you lusty looks", false);
