	...
	public class HellHoundScene extends BaseContent
	{
		...
		public function hellhoundRapesPlayer():void
		{
			...
			if (player.vaginas.length > 0 && player.lowerBody != LOWER_BODY_TYPE_NAGA) {
				...
				if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS_WET) outputText("The pause gives your " + player.vaginaDescript(0) + " time to moisten, ensuring the next thrust won't be quite as rough.  ");
...
//Merae's Lethicite
		...
		public function hellHoundPropahRape():void
		{
			...
			//--- ELSE (CORRUPTION >= 20) ---
				...
				if (player.isPureEnough(40) && player.findPerk(PerkLib.Masochist) < 0) {
				...
				//[if not corrupt]
				if (player.isPureEnough(40)) dynStats("tou", -2, "cor", 1);
				//[if corrupt]
				else dynStats("cor", 1.5);
				//Preggers chance!
				player.knockUp(PregnancyStore.PREGNANCY_HELL_HOUND, PregnancyStore.INCUBATION_HELL_HOUND, 101);
