...
private function keltEagerlyGetNaked():void {
	...
	if (player.butt.rating < 6) outputText("\"<i>Well, someone works out!  Got a nice, tight little ass there! Probably a little too tight, huh?  Don't worry, you can tell me... do the mean old monsters hurt when they fuck that poor little ass of yours? HA!</i>\"\r\r");
	//(Medium, Large to Heavy:
	else if (player.butt.rating < 13) outputText("\"<i>Hey, nice little cushion you got back here!  Do you get that from sitting around all day?  Nah, can't be that.  I'm sure you get more than a workout on this baby... of course, cock is no substitute for a little exercise, you know!</i>\"\r\r");
	...
	if (player.totalCocks() == 0) outputText("\"<i>Well, aren't you a pretty little thing, all together.  You and I will get along real nice, that's for sure!</i>\"\r\r");
		...
		//(Any Cock: 
		outputText("and almost inevitably, his eyes drop down to your " + player.cockDescript(0) + ".  He snorts, ");
		//(Human Cock:
...
private function keltMainEncounter3():void {
	...
	//(Clothed)
	if (!player.hasStatusEffect(StatusEffects.NakedOn)) {
		outputText(images.showImage("kelt-archery"));
	...
	//NAKERS
		...
		//(Naked, 60% Chance)
		if (temporary <= 2) {
			outputText(images.showImage("event-archery"));
		...
		if (temporary == 3) {
			...
			outputText("\"<i>Isn't it obvious?</i>\" he asks, grinning down at you in sardonic amusement. \"<i>You were way off balance.  Of course, I would be too, if I had a couple of extra pounds of fat hanging off my chest.</i>\"\r\r");
...
private function keltCentaurBadEnd():void {
	...
	if (player.gender == 1) {
		...
		outputText(images.showImage("kelt-lost-male"));
	...
	if (player.gender >= 2) {
		...
		outputText("<b>... It was for him to fuck you!</b>\r\r");
		outputText(images.showImage("kelt-lost-female"));
...
private function fuckKeltsShitUp():void {
	...
	clearOutput();
	//If naga folks
	if (player.face.type == Face.SNAKE_FANGS && player.tongue.type == Tongue.SNAKE && player.lowerBody.type == LowerBody.NAGA) {
