package classes.Scenes.Monsters.pregnancies 
{
	...
	public class PlayerBunnyPregnancy implements VaginalPregnancy 
	{
		 ...
		 */
		public function PlayerBunnyPregnancy(pregnancyProgression:PregnancyProgression, output:GuiOutput, mutations:Mutations)
		{
		...
		public function updateVaginalPregnancy():Boolean 
		{
			...
			if (player.pregnancyIncubation === 156) {
				...
				if (player.cor >= 40 && player.cor < 75) output.text("You find yourself wondering what giving birth to bunny-girls is like.</b>");
				if (player.cor >= 75) output.text("You dreamily wonder if you could find a bunny willing to put more than two eggs inside you at once.</b>");
			...
			if (player.pregnancyIncubation === 72) {
				...
				if (player.cor >= 75) output.text("and you're eager to give birth so you'll be able to get pregnant again.</b>");
				output.text("\n");
			...
			if (player.pregnancyIncubation === 48) {
				...
				if (player.cor >= 75) output.text("You find yourself daydreaming about birthing bunnies repeatedly, each time being re-impregnated with dozens of eggs from your lusty adolescent children.</b>\n");
				displayedUpdate = true;
			}
		...
		public function vaginalBirth():void 
		{
			...
			if (player.vaginalCapacity() < 60) output.text("It hurts a little as your cervix starts to stretch wide");
			else output.text("It actually feels kind of nice as your cervix is stretched wide");
			...
			if (player.lactationQ() > 500) output.text(", growing fat from all the milk");
