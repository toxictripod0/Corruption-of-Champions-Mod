...
public class Combat extends BaseContent
	{
			...
			if (damage > 0) {
				...
				if (monster.lustVuln > 0) {
					...
					if (player.weapon == weapons.L_WHIP) {
						...
						else outputText("\n" + monster.capitalA + monster.short + " shiver and moan involuntarily from the flaming whip's touches.");
		...
		//		doNext(combatMenu);
			...
			if (player.tailType == Tail.RACCOON && player.ears.type == Ears.RACCOON && player.findPerk(PerkLib.Runner) >= 0) escapeMod -= 25;
			...
			if (player.spe > rand(monster.spe + escapeMod)) {
				...
				else if (player.tailType == Tail.RACCOON && player.ears.type == Ears.RACCOON && player.findPerk(PerkLib.Runner) >= 0) {
			...
			//FAIL FLEE
				...
				//fail
				else if (player.tailType == Tail.RACCOON && player.ears.type == Ears.RACCOON && player.findPerk(PerkLib.Runner) >= 0) outputText("Using your running skill, you build up a head of steam and jump, but before you can clear the ground more than a foot, your opponent latches onto you and drags you back down with a thud!");
				//Nonflyer messages
				else {
					//Huge balls messages
