package classes.Scenes.NPCs.pregnancies 
{
	...
	public class PlayerOviElixirPregnancy implements VaginalPregnancy
	{
		 ...
		 */
		public function PlayerOviElixirPregnancy(pregnancyProgression:PregnancyProgression, output:GuiOutput) 
		{
			this.output = output;
		...
		private function vaginaCheck():Boolean {
		...
		public function vaginalBirth():void 
		{
			...
			if (player.statusEffectv2(StatusEffects.Eggs) === 0) {
				...
				//High quantity
					...
					if (player.getClitLength() > 5) output.text("jerking on your huge clitty as if it were a cock, moaning and panting as each egg slides free of your diminishing belly.  You lubricate it with a mix of your juices and the slime until ");
					if (player.getClitLength() > 2 && player.getClitLength() <= 5) output.text("playing with your over-large clit as if it were a small cock, moaning and panting as the eggs slide free of your diminishing belly.  You spread the slime and cunt juice over it as you tease and stroke until ");
					if (player.getClitLength() <= 2) output.text("pulling your folds wide and playing with your clit as another egg pops free from your diminishing belly.  You make wet 'schlick'ing sounds as you spread the slime around, vigorously frigging yourself until "); 
			...
			//Large egg scene
				...
				if (player.vaginas[0].vaginalLooseness < VaginaClass.LOOSENESS_LOOSE) output.text("stretching your tight cunt painfully, the lips opening wide ");
				if (player.vaginas[0].vaginalLooseness >= VaginaClass.LOOSENESS_LOOSE && player.vaginas[0].vaginalLooseness <= VaginaClass.LOOSENESS_GAPING_WIDE) output.text("temporarily stretching your cunt-lips wide-open ");
				if (player.vaginas[0].vaginalLooseness > VaginaClass.LOOSENESS_GAPING_WIDE) output.text("parting your already gaping lips wide ");
				...
				if (player.getClitLength() > 5) output.text("your massive cock-like clit, jacking it off with the slimy birthing fluids as lube.   It pulses and twitches in time with your heartbeats, its sensitive surface overloading your fragile mind with pleasure.  ");
				if (player.getClitLength() > 2 && player.getClitLength() <= 5) output.text("your large clit like a tiny cock, stroking it up and down between your slime-lubed thumb and fore-finger.  It twitches and pulses with your heartbeats, the incredible sensitivity of it overloading your fragile mind with waves of pleasure.  ");
				if (player.getClitLength() <= 2) output.text("your " + player.vaginaDescript(0) + " by pulling your folds wide and playing with your clit.  Another egg pops free from your diminishing belly, accompanied by an audible burst of relief.  You make wet 'schlick'ing sounds as you spread the slime around, vigorously frigging yourself.  ");
			...
			if (player.hasStatusEffect(StatusEffects.AteEgg)) output.text("but you remember the effects of the last one you ate.\n</b>");
			else output.text("but your body's intuition reminds you they shouldn't be fertile, and your belly rumbles with barely contained hunger.\n</b>");
