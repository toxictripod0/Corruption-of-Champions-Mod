		...
		public function updateVaginalPregnancy():Boolean 
		{
			...
			if (player.pregnancyIncubation === 216) {
				if (kGAMECLASS.flags[kFLAGS.JOJO_BIMBO_STATE] >= 3 && player.pregnancyType === PregnancyStore.PREGNANCY_JOJO) { //Bimbo Jojo, stage 3
					...
					if (kGAMECLASS.flags[kFLAGS.JOJO_STATUS] > 0) {
						...
						if (player.cor >= 40 && player.cor < 75) output.text("Considering the size of the creatures you've fucked, you hope it doesn't hurt when it comes out.</b>");
						if (player.cor >= 75) output.text("You think dreamily about the monstrous cocks that have recently been fucking you, and hope that your offspring inherit such a pleasure tool.</b>");
					}
					else {
						output.text("</b>");
					}
			...
			if (player.pregnancyIncubation === 120) {
				if (kGAMECLASS.flags[kFLAGS.JOJO_BIMBO_STATE] >= 3 && player.pregnancyType === PregnancyStore.PREGNANCY_JOJO) { //Bimbo Jojo, stage 5
					...
					output.text("\n");
					if (player.findPerk(PerkLib.Feeder) >= 0) output.text("You grin at Joy's idea, but you can't simply mash her against your breasts and nurse her without some teasing first.");
			...
			if (player.pregnancyIncubation === 72) {
				...
				if (player.cor >= 75) output.text("and you're eager to give birth, so you can get impregnated again by monstrous cocks unloading their corrupted seed directly into your eager womb.</b>");
				output.text("\n");
			...
			if (player.pregnancyIncubation === 48) {
				...
				if (player.cor >= 75) output.text("You find yourself daydreaming about birthing hundreds of little babies, and lounging around while they nurse non-stop on your increasingly sensitive breasts.</b>\n");				
			}
