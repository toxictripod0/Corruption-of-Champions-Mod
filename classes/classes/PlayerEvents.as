	...
	public class PlayerEvents extends BaseContent implements TimeAwareInterface {
		...
		public function timeChange():Boolean {
			...
			if (player.findPerk(PerkLib.SlimeCore) >= 0) { //Lose slime core perk
				if (player.vaginalCapacity() < 9000 || player.skinAdj != "slimy" || player.skinDesc != "skin" || player.lowerBody.type != LowerBody.GOO) {
			...
			if (player.hasKeyItem("Ruby Heart") >= 0) { //Regain slime core
				if (player.hasStatusEffect(StatusEffects.SlimeCraving) && player.findPerk(PerkLib.SlimeCore) < 0 && player.isGoo() && player.gooScore() >= 4 && player.vaginalCapacity() >= 9000 && player.skinAdj == "slimy" && player.skinDesc == "skin" && player.lowerBody.type == LowerBody.GOO) {
