package classes 
{
	...
	public class CharSpecial extends BaseContent
	{
		...
		private function customCharlie():void {
			...
			player.wingType = Wings.TYPE_FEATHERED_LARGE;
	...
	//		if (player.findPerk(PerkLib.WizardsEndurance) < 0) player.createPerk(PerkLib.WizardsEndurance,30,0,0,0);
		...
		private function customEtis():void {
			...
			player.vaginas[0].vaginalWetness = VAGINA_WETNESS_WET; // wet
			player.vaginas[0].virgin = false;
			...
			player.tailType = TAIL_TYPE_FOX; // soft fur feels so lovely...
			player.tailVenom = 9;			
			...
			player.horns = 8;
			player.wingType = Wings.TYPE_DRACONIC_LARGE; // wings! to fly!
