...
public function encounterASandTarp():void {
	...
	if (flags[kFLAGS.SANDTRAP_LOSS_REPEATS] >= 2 && player.eyes.type == Eyes.BLACK_EYES_SAND_TRAP && player.wingType == Wings.GIANT_DRAGONFLY) {
		sandTrapBadEnd();
		return;
	}
...
private function dickwieldersLoseToSandTarps():void {
	clearOutput();
	spriteSelect(SpriteDb.s_sandtrap);	
	outputText(images.showImage("sandtrap-male-lose-get-analraped"));
...
private function chicksLoseToSandTarp():void {
	...
	outputText(images.showImage("sandtrap-femele-lose-get-analraped"));
...
private function genderlessLoss():void {
	...
	outputText(images.showImage("sandtrap-genderless-lose-get-analraped"));	
...
public function birfSandTarps():void {
	spriteSelect(SpriteDb.s_sandtrap);
	if (player.isFemale()) outputText(images.showImage("sandtrap-female-vagbirth"));
	else outputText(images.showImage("sandtrap-male-analbirth"));
...
private function useSandTarpsHands():void {
	...
	//Male: 
	if (player.gender == 1) {
		outputText(images.showImage("sandtrap-male-win-get-hj"));
	...
	//Female: 
		...
		outputText(images.showImage("sandtrap-female-win-get-hj"));
	...
	//Herm: 
		...
		outputText(images.showImage("sandtrap-herm-win-get-hj"));
...
private function rideDatSandTarpLikeIts1999():void {
	...
	outputText(images.showImage("sandtrap-female-win-rideit"));
...
private function stickWangInSandgina():void {
	...
	outputText(images.showImage("sandtrap-male-win-vagsex"));
...
private function nagaThreesomeWithSandTrap():void {
	...
	//Male/Herm: 
	if (player.hasCock()) {
		outputText(images.showImage("sandtrap-nagamale-win-3some-get-bj"));
	...
	//Female: 
		...
		outputText(images.showImage("sandtrap-nagafemale-win-3some-get-cunning"));
...
private function sandTrapBadEndFinale():void {
	spriteSelect(SpriteDb.s_sandtrap);
	outputText(images.showImage("sandtrap-badend"));
